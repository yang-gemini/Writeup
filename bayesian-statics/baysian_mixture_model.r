...
n.all = length(y)
Y=matrix(y[4:n.all],ncol=1) ## y_{p+1:T}
Fmtx=matrix(c(y[3:(n.all - 1)],y[2:(n.all - 2)], y[1:(n.all - 3)]),nrow=p,byrow=TRUE) ## design matrix F
n=length(Y) ## T-p
...
dev.off()
# Calculate the residuals: Observed (Y) minus Posterior Mean Prediction
residuals_mix <- as.vector(Y) - summary.y[2, ]
png(filename = "./images/mixture-model-residual.png", 
    width = 1920,           # Breite in Pixeln
    height = 1080,           # Höhe in Pixeln
    res = 150)              # Auflösung erhöhen für scharfen Text (DPI)
plot(residuals_mix, type = 'h', col = "darkred",
     xlab = "Time", ylab = "Residuals (Observed - Predicted)",
     main = "Residual Plot for Mixture AR(3) Predictions")
abline(h = 0, col = "blue", lty = 2, lwd = 2) # Reference baseline at 0
dev.off()